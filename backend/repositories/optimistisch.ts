...
export async function optimistischOpslaan(
    ...
    const [bestaande] = await pool.execute<RowDataPacket[]>(
        `SELECT versie FROM ${tabel} WHERE ${where}`, sleutelWrd,
    );
    ...
    try {
        ...
        if ((e as { code?: string }).code === 'ER_DUP_ENTRY') throw new AppError(CONFLICT_BERICHT, 409);
        throw e;
    }
    return leesMeta(pool, tabel, where, sleutelWrd);
}

async function leesMeta(pool: Pool, tabel: string, where: string, sleutelWrd: Waarde[]): Promise<OpslaanResultaat> {
    const [rows] = await pool.execute<RowDataPacket[]>(
        `SELECT versie, auteur, DATE_FORMAT(bijgewerkt_op, '%Y-%m-%dT%H:%i:%s') AS bijgewerkt_op
         FROM ${tabel} WHERE ${where}`, sleutelWrd,
    );
    const r = (rows[0] as { versie?: number; auteur?: string | null; bijgewerkt_op?: string | null }) ?? {};
    return { versie: Number(r.versie ?? 0), auteur: r.auteur ?? null, bijgewerkt_op: r.bijgewerkt_op ?? null };
}
