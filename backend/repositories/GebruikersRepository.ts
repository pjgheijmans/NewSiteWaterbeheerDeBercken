...
const DEFAULT_GEBRUIKERS: Array<{
    voornaam: string;
    achternaam: string;
    inlognaam: string;
    wachtwoord: string;
...
export class GebruikersRepository implements IGebruikersRepository {
    ...
    async getMetRecht(domein: Domein, minNiveau: Rechtniveau): Promise<GebruikerNaam[]> {
        ...
        const toegestaan = NIVEAU_ORDE.slice(NIVEAU_ORDE.indexOf(minNiveau)).filter(
            (n) => n !== 'geen',
        );
    ...
    async remove(id: string): Promise<void> {
        ...
        await this.pool.execute<ResultSetHeader>('DELETE FROM gebruikers WHERE id = ?', [id]);
    }
    ...
    private async _laadRechten(
        gebruikerId: number,
    ): Promise<{ rechten: RolRechten; magHistorie: boolean; rolNamen: string[] }> {
        const [rows] = await this.pool.execute<RowDataPacket[]>(
