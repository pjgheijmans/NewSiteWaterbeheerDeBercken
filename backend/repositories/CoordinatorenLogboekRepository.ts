...
export class CoordinatorenLogboekRepository implements ICoordinatorenLogboekRepository {
     ...
     */
    async save(datum: string, tijdstip: string, tekst: string, auteur: string | null): Promise<LogboekSaveResult | null> {
    ...
    async getDatumById(id: string): Promise<string | null> {
        const [rows] = await this.pool.execute<RowDataPacket[]>(
            "SELECT DATE_FORMAT(datum, '%Y-%m-%d') AS datum FROM coordinatoren_logboek WHERE id = ?", [id]
        );
    ...
    async deleteById(id: string): Promise<void> {
        await this.pool.execute<ResultSetHeader>(
            'DELETE FROM coordinatoren_logboek WHERE id = ?', [id]
        );
    }
}
