...
export class RollenRepository implements IRollenRepository {
    ...
    async getAll(): Promise<Rol[]> {
        ...
        for (const row of rows as Array<{ id: number; naam: string; mag_historie_bewerken: number; domein: Domein | null; niveau: Rechtniveau | null }>) {
            ...
            if (!rol) {
                ...
                rol = { id: row.id, naam: row.naam, mag_historie_bewerken: !!row.mag_historie_bewerken, rechten };
    ...
    async create(naam: string): Promise<void> {
        await this.pool.execute<ResultSetHeader>(
            'INSERT INTO rollen (naam, mag_historie_bewerken) VALUES (?, 0)', [naam]
        );
    }
