...
const ALL_DATA_TABLES = [
    ...
    'verwarmings_systeem_diep_ondiep', 'limieten', 'gebruikers',
];
...
export class DatabaseRepository implements IDatabaseRepository {
    ...
    async truncateAll(): Promise<void> {
        ...
        for (const tabel of ALL_DATA_TABLES) {
            try { await this.pool.query(`TRUNCATE TABLE ${tabel}`); }
            catch (err) { console.warn(`truncateAll: skipping ${tabel}: ${(err as Error).message.slice(0, 80)}`); }
        }
        await this.pool.query('SET FOREIGN_KEY_CHECKS = 1');
    }
