...
export class AuthController {
    ...
    private async login(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!gebruiker) {
                res.status(401).json({ error: 'Onjuiste inlognaam of wachtwoord' });
                return;
            }
    ...
    private ingelogd(req: Request, res: Response): void {
        if (req.session?.gebruiker) res.json({ ingelogd: true, gebruiker: req.session.gebruiker });
        else res.json({ ingelogd: false });
    }
}
