...
export class RollenController {
    ...
    constructor(private readonly repo: IRollenRepository) {
        ...
        this.router.post('/',      checkAuth, vereist('beheer', 'schrijven'), valideerBody(rolCreateSchema), this.create.bind(this));
        this.router.put('/:id',    checkAuth, vereist('beheer', 'schrijven'), valideerBody(rolUpdateSchema), this.update.bind(this));
        this.router.delete('/:id', checkAuth, vereist('beheer', 'schrijven'), this.remove.bind(this));
    }
    ...
    private async create(req: Request, res: Response, next: NextFunction): Promise<void> {
        try { await this.repo.create(req.body.naam as string); res.json({ status: 'success' }); }
        catch (err) { next(err); }
    }

    private async update(req: Request, res: Response, next: NextFunction): Promise<void> {
        try { await this.repo.update(String(req.params['id']), req.body as RolInput); res.json({ status: 'success' }); }
        catch (err) { next(err); }
    }
