...
export class DatabaseController {
    ...
    constructor(private readonly service: IDatabaseService) {
        ...
        this.router.post(
            ...
            checkAuth,
            vereist('beheer', 'schrijven'),
            this.verwijderAlles.bind(this),
        );
        this.router.post(
            '/initialiseer',
            checkAuth,
            vereist('beheer', 'schrijven'),
            this.initialiseer.bind(this),
        );
    }
    ...
    private async truncate(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        if (!TRUNC_TABLES.includes(tabel)) {
            res.status(400).json({ error: 'Ongeldige tabelnaam' });
            return;
        }
        try {
    ...
    private async exportCsv(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            if (csv === null) {
                res.status(404).json({ error: 'Tabel is leeg, niets te exporteren' });
                return;
            }
            res.setHeader('Content-Type', 'text/csv; charset=utf-8');
            res.setHeader(
                'Content-Disposition',
                `attachment; filename=export_${tabel}_${new Date().toISOString().split('T')[0]}.csv`,
            );
    ...
    private async initialiseer(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            res.json({
                status: 'success',
                message: 'Database geïnitialiseerd met standaardwaarden.',
            });
        } catch (err) {
            next(err);
        }
