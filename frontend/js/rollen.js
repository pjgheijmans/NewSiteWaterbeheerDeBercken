...
class RollenModule {
    ...
    static get DOMEINEN() {
        return [['beheer', 'Beheer'], ['waterbeheer', 'Waterbeheer'], ['coordinator', 'Coördinator']];
    }

    /** Niveaus in oplopende macht: [waarde, label]. */
    static get NIVEAUS() {
        return [['geen', 'Geen'], ['lezen', 'Lezen'], ['schrijven', 'Schrijven']];
    }
    ...
    async laad() {
        ...
        } catch { this.app.ui.toonBericht('Fout bij laden rollen.', 'fout'); }
    }

    /** @private Bouw een <select> voor het niveau van een domein. */
    _selectHtml(domein, niveau) {
        const opts = RollenModule.NIVEAUS
            .map(([v, l]) => `<option value="${v}" ${niveau === v ? 'selected' : ''}>${l}</option>`)
            .join('');
    ...
    _rij(rol) {
        ...
        const cellen = RollenModule.DOMEINEN
            .map(([d]) => `<td>${this._selectHtml(d, (rol.rechten && rol.rechten[d]) || 'geen')}</td>`)
            .join('');
        tr.innerHTML = `
    ...
    _scheduleAutoSave(id) {
        ...
        timers[id] = setTimeout(() => { this._setSaveStatus('saving'); this._wijzigRol(id); }, 1200);
    }
    ...
    async _wijzigRol(id) {
        ...
        if (!rij) return;
        const rechten = {};
        rij.querySelectorAll('.r-niveau').forEach(sel => { rechten[sel.dataset.domein] = sel.value; });
        const payload = {
        ...
        try {
            ...
            else { this._setSaveStatus('error'); this.app.ui.toonBericht('Fout bij opslaan rol.', 'fout'); }
        } catch { this._setSaveStatus('error'); }
    }
    ...
    async voegRolToe() {
        ...
        if (!naam) { this.app.ui.toonBericht('Geef een rolnaam op.', 'fout'); return; }
        const res = await this.app.api.call('/api/rollen', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async verwijderRol(id) {
        if (!(await this.app.ui.bevestig({ titel: 'Rol wissen', tekst: 'Weet u zeker dat u deze rol wilt wissen? Gebruikers met deze rol verliezen de bijbehorende rechten.', bevestig: 'Wissen', gevaar: true }))) return;
        const res = await this.app.api.call(`/api/rollen/${id}`, { method: 'DELETE' });
        if (res.ok) { this.app.ui.toonBericht('Rol gewist.', 'succes'); this.laad(); }
    }
}
