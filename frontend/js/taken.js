...
class TakenModule {
    ...
    async werkBadgeBij(datum) {
        if (!datum) return;
        try { this._items = await this._haalOp(datum); this._zetBadges(); }
        catch (f) { console.error('Fout bij verversen taken-badges:', f); }
    }

    /** Laad data en render de Taken-subtab van een bad-pagina. */
    async laadBadTaken(pagina, datum) {
        if (!datum) return;
        try { this._items = await this._haalOp(datum); this._zetBadges(); this._renderBad(pagina); }
        catch (f) { console.error('Fout bij laden taken:', f); }
    }
    ...
    async toggle(sleutel, voltooid) {
        ...
        try {
            if (item.bron.type === 'rondetaak') {
                ...
                await Promise.all(item.bron.ids.map(id =>
        ...
        } catch (f) { console.error('Fout bij bijwerken taak:', f); }
    }
    ...
    _renderBad(pagina) {
        ...
        if (!el) return;
        const items = this._items.filter(i => i.pagina === pagina).sort((a, b) => {
