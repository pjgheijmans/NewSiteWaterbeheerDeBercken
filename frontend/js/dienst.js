...
class DienstModule {
    ...
    async _laadWaterbeheerders() {
        ...
        try {
            ...
            if (lijst && Array.isArray(namen)) {
                lijst.innerHTML = namen.map(n => `<option value="${String(n).replace(/"/g, '&quot;')}">`).join('');
                this._waterbeheerdersGeladen = true;
            }
        } catch (f) { console.error('Kon waterbeheerders niet laden', f); }
    }
    ...
    async laadDienst(datum) {
        ...
        } catch (f) { console.error('Fout bij laden dienst:', f); }
        this._updateSamenvatting();
