const gebruikersSaveTimers = {};

function setGebruikersSaveStatus(status) {
...
function scheduleAutoSaveGebruiker(id) {
    if (gebruikersSaveTimers[id]) clearTimeout(gebruikersSaveTimers[id]);
    setGebruikersSaveStatus('pending');
    gebruikersSaveTimers[id] = setTimeout(async () => {
        setGebruikersSaveStatus('saving');
        await wijzigGebruiker(id);
    }, 1200);
}
    ...
    try {
        ...
        gebruikers.forEach(g => {
            ...
            tr.querySelectorAll('input, select').forEach(el => {
                el.addEventListener('input', () => scheduleAutoSaveGebruiker(g.id));
                el.addEventListener('change', () => scheduleAutoSaveGebruiker(g.id));
            });
        });
...
async function voegGebruikerToe() {
    ...
    if (res.ok) {
        toonBericht('Gebruiker toegevoegd!', 'succes');
        ['g-voornaam', 'g-achternaam', 'g-inlognaam', 'g-wachtwoord'].forEach(id => document.getElementById(id).value = '');
        laadGebruikers();
...
async function wijzigGebruiker(id) {
    ...
    if (res.ok) {
        setGebruikersSaveStatus('saved');
    } else {
        setGebruikersSaveStatus('error');
        toonBericht('Fout bij opslaan gebruiker.', 'fout');
    }
}
