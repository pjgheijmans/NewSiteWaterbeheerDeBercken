...
class TrendModule {
    ...
    wisselTrendTab(subtab) {
        ...
        ['meetwaarden', 'verbruik'].forEach((s) => {
            ...
            document.getElementById(`trend-${s}-content`).style.display =
                s === subtab ? 'block' : 'none';
        });
    }
    ...
    initTrendDatums() {
        ...
        if (!document.getElementById('trend-van').value)
            document.getElementById('trend-van').value = fmt(van);
        if (!document.getElementById('trend-tot').value)
            document.getElementById('trend-tot').value = fmt(tot);
    }
    ...
    async laadTrendData() {
        ...
        if (!van || !tot) {
            this.app.ui.toonBericht('Vul een geldig datumbereik in.', 'fout');
            return;
        }
        ...
        else await this._laadTrendVerbruik(van, tot);
    ...
    async _laadTrendMetingen(van, tot) {
        try {
            ...
            const grootLabels = [
                ...new Set([...diep, ...ondiep].map((r) => this._datumStr(r.datum))),
            ].sort();
            const peuterLabels = peuter.map((r) => this._datumStr(r.datum));

            const haal = (arr, veld, lbls) =>
                lbls.map((d) => {
            ...
            this._maakLineChart('chart-ph-groot', grootLabels, [
                ds('Diep', diep, 'ph_waarde', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'ph_waarde', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-chloor-groot', grootLabels, [
                ds('Diep', diep, 'chloor_waarde', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'chloor_waarde', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-temp-groot', grootLabels, [
                ds('Diep', diep, 'temperatuur', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'temperatuur', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-flow-groot', grootLabels, [
                ds('Diep', diep, 'flow', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'flow', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-filterin-groot', grootLabels, [
                ds('Diep', diep, 'filter_druk_in', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'filter_druk_in', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-filteruit-groot', grootLabels, [
                ds('Diep', diep, 'filter_druk_uit', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'filter_druk_uit', grootLabels, '#28a745'),
            ]);
            this._maakLineChart('chart-kath-groot', grootLabels, [
                ds('Diep', diep, 'kathodische_bescherming', grootLabels, '#007BFF'),
                ds('Ondiep', ondiep, 'kathodische_bescherming', grootLabels, '#28a745'),
            ]);

            this._maakLineChart('chart-ph-peuter', peuterLabels, [
                ds('Peuterbad', peuter, 'ph_waarde', peuterLabels, '#fd7e14'),
            ]);
            this._maakLineChart('chart-chloor-peuter', peuterLabels, [
                ds('Peuterbad', peuter, 'chloor_waarde', peuterLabels, '#fd7e14'),
            ]);
            this._maakLineChart('chart-flow-peuter', peuterLabels, [
                ds('Peuterbad', peuter, 'flow', peuterLabels, '#fd7e14'),
            ]);
            this._maakLineChart('chart-filterin-peuter', peuterLabels, [
                ds('Peuterbad', peuter, 'filter_druk_in', peuterLabels, '#fd7e14'),
            ]);
        } catch (f) {
            console.error('Fout trend metingen:', f);
            this.app.ui.toonBericht('Fout bij laden grafiek.', 'fout');
        }
    }
    ...
    async _laadTrendVerbruik(van, tot) {
        try {
            ...
            this._maakLineChart('chart-water-groot', labels, [
                lijn('Diep (m³)', alg, 'water_diep', '#007BFF'),
                lijn('Ondiep (m³)', alg, 'water_ondiep', '#28a745'),
                { ...lijn('Totaal (m³)', alg, 'water_totaal', '#6c757d'), borderDash: [5, 5] },
            ]);
            this._maakLineChart('chart-elektriciteit', labels, [
                lijn('Nacht (kWh)', alg, 'elektriciteit_nacht', '#6f42c1'),
                lijn('Dag (kWh)', alg, 'elektriciteit_dag', '#e83e8c'),
            ]);
            this._maakLineChart('chart-gas', labels, [lijn('Gas (m³)', alg, 'gas', '#fd7e14')]);
            this._maakLineChart('chart-water-peuter', peuterLabels, [
                lijn('Water (m³)', peuter, 'water', '#fd7e14'),
            ]);
            this._maakLineChart('chart-chem-chloor-groot', labels, [
                lijn('Chloor', alg, 'chemicalien_chloor', '#17a2b8'),
            ]);
            this._maakLineChart('chart-chem-zwavel-groot', labels, [
                lijn('Zwavelzuur', alg, 'chemicalien_zwavelzuur', '#dc3545'),
            ]);
            this._maakLineChart('chart-chem-chloor-peuter', peuterLabels, [
                lijn('Chloor', peuter, 'chemicalien_chloor', '#17a2b8'),
            ]);
            this._maakLineChart('chart-chem-zwavel-peuter', peuterLabels, [
                lijn('Zwavelzuur', peuter, 'chemicalien_zwavelzuur', '#dc3545'),
            ]);
        } catch (f) {
            console.error('Fout trend verbruik:', f);
            this.app.ui.toonBericht('Fout bij laden grafiek.', 'fout');
        }
    }
}
