...
class DatabaseModule {
            ...
            try {
                ...
                if (res.ok)
                    this.app.ui.toonBericht(
                        `CSV succesvol geïmporteerd in '${tabelnaam}'!`,
                        'succes',
                    );
                else this.app.ui.toonBericht(`Import mislukt: ${data.error}`, 'fout');
            } catch {
                this.app.ui.toonBericht('Verbindingsfout tijdens import.', 'fout');
            }
            inputElement.value = '';
        };
    ...
    async leegmakenTabel(tabelnaam) {
        ...
        try {
            const res = await this.app.api.call(`/api/database/truncate/${tabelnaam}`, {
                method: 'POST',
            });
            const data = await res.json();
            if (res.ok) {
            ...
            this.app.ui.toonBericht('Verbindingsfout met server.', 'fout');
        }
    }
    ...
    async verwijderDatabase() {
        ...
        try {
            const res = await this.app.api.call('/api/database/verwijder-alles', {
                method: 'POST',
            });
            const data = await res.json();
            if (res.ok) {
                await this.app.ui.meld('Database volledig gewist. U wordt uitgelogd.');
    ...
    async maakNieuweDatabase() {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                await this.app.ui.meld('Nieuwe database aangemaakt. U wordt uitgelogd.');
                window.location.reload();
            } else this.app.ui.toonBericht(`Fout: ${data.error}`, 'fout');
        } catch {
            this.app.ui.toonBericht('Verbindingsfout met server.', 'fout');
