...
class MetingenModule {
    ...
    wisselBadPagina(pagina) {
        ...
        document.getElementById('waterbeheer-grote-baden-content').style.display =
            pagina === 'grote-baden' ? 'block' : 'none';
        document.getElementById('waterbeheer-peuterbad-content').style.display =
            pagina === 'peuterbad' ? 'block' : 'none';
        document.getElementById('waterbeheer-logboek-content').style.display =
            pagina === 'logboek' ? 'block' : 'none';
    ...
    wisselSubtab(subtab) {
        ...
        ['meetwaarden', 'verbruik', 'verwarmingssysteem', 'bezoekers', 'taken'].forEach((s) => {
            ...
            document.getElementById(`subtab-${s}-content`).style.display =
                s === subtab ? 'block' : 'none';
        });
        if (subtab === 'verbruik' || subtab === 'verwarmingssysteem')
            this.app.verbruik.laadWaterbeheerVelden();
        if (subtab === 'bezoekers') this.laadBezoekers();
        if (subtab === 'taken')
    ...
    wisselCoordSubtab(subtab) {
        ...
        document.getElementById('coordinatoren-blokken-content').style.display =
            subtab === 'metingen' ? 'block' : 'none';
        document.getElementById('coordinatoren-checklist-content').style.display =
            subtab === 'checklist' ? 'block' : 'none';
        document.getElementById('coordinatoren-daggegevens-content').style.display =
            subtab === 'daggegevens' ? 'block' : 'none';
        document.getElementById('coordinatoren-logboek-content').style.display =
            subtab === 'logboek' ? 'block' : 'none';
        ...
        if (subtab === 'logboek')
            this.app.logboek.laadLogboek(
                datum,
                'coordinatoren-logboek-blokken',
                '/api/coordinatoren/logboek',
            );
    }
        ...
        try {
            ...
            if (huidigeRol === 'waterbeheer') {
                ...
                if (
                    ...
                    this.app.state.huidigeSubtab === 'taken'
                ) {
    ...
    async laadGebondenChloor() {
        ...
        try {
            ...
            console.error('Fout bij laden gebonden chloor:', f);
        }
    }
        ...
        try {
            ...
            open.forEach((actie) => {
                (veldKaart[`${actie.actie_type}|${actie.bad_naam}`] || []).forEach((inputId) => {
                    ...
                    ind.className = 'actie-indicator';
                    ind.title = actie.beschrijving;
                    ind.textContent = '⚠';
                    ...
                    const knop =
                        MetingenModule.ACTIE_SUBTAB_KNOP[input.closest('[id$="-content"]')?.id];
            ...
            gesloten.forEach((actie) => {
                (veldKaart[`${actie.actie_type}|${actie.bad_naam}`] || []).forEach((inputId) => {
                    ...
                    const tijdstip = actie.opgelost_op
                        ? String(actie.opgelost_op).slice(0, 16).replace('T', ' ')
                        : '';
    ...
    werkVolledigheidBij() {
        ...
        const grote =
            huidigeBadPagina === 'grote-baden'
                ? this._groteBadenOnvolledigUitDom()
                : this._groteBadenOnvolledigUitData();
        const peuter =
            huidigeBadPagina === 'peuterbad'
                ? this._peuterbadOnvolledigUitDom()
                : this._peuterbadOnvolledigUitData();
    ...
    _groteBadenOnvolledigUitDom() {
        ...
        const meet = ['diep', 'ondiep'].some((lb) =>
    ...
    _groteBadenOnvolledigUitData() {
        ...
        const meet = ['Diep', 'Ondiep'].some((bad) => {
            ...
            return OpslaanModule.meetwaardenOnvolledig({
                ph_waarde: L(r.ph_waarde),
                chloor_waarde: L(r.chloor_waarde),
                temperatuur: L(r.temperatuur),
                flow: L(r.flow),
                filter_druk_in: L(r.filter_druk_in),
                filter_druk_uit: L(r.filter_druk_uit),
                kathodische_bescherming: L(r.kathodische_bescherming),
            });
        });
        const v = this.app.state.gecachteVerbruik || {};
        const verbruik = VerbruikModule.verbruikOnvolledig({
            water_diep: L(v.water_diep),
            water_ondiep: L(v.water_ondiep),
            water_totaal: L(v.water_totaal),
    ...
    _peuterbadOnvolledigUitData() {
        ...
        const payload = {
            ...
            chloor_waarde: L(r.chloor_waarde),
            flow: L(r.flow),
            filter_druk: L(r.filter_druk ?? r.filter_druk_in),
            water: L(r.water),
            chemicalien_chloor: L(r.chemicalien_chloor),
    ...
    _bouwTabelOp(data) {
        ...
        if (huidigeRol === 'waterbeheer' && huidigeBadPagina === 'grote-baden') {
            ...
            ['meetwaarden', 'verbruik', 'verwarmingssysteem', 'bezoekers'].forEach((s) => {
                document
                    .getElementById(`subtab-${s}`)
                    .classList.toggle('actief', s === huidigeSubtab);
                document.getElementById(`subtab-${s}-content`).style.display =
                    s === huidigeSubtab ? 'block' : 'none';
            });

            ['diep', 'ondiep'].forEach((b) => {
                ['ph', 'chloor', 'temp', 'flow'].forEach((f) => {
                    document.getElementById(`${f}-${b}`).value = '';
                });
                ['filter-in', 'filter-uit', 'kath'].forEach((f) => {
                    document.getElementById(`${f}-${b}`).value = '';
                });
            });
        ...
        if (huidigeRol === 'waterbeheer' && huidigeBadPagina === 'peuterbad') {
            ...
            [
                'peuterbad-ph',
                'peuterbad-chloor',
                'peuterbad-filterdruk',
                'peuterbad-flow',
                'peuterbad-water',
                'peuterbad-chemicalien-chloor',
                'peuterbad-chemicalien-zwavelzuur',
            ].forEach((id) => {
                const el = document.getElementById(id);
                if (el) el.value = '';
            });
            const meting = Array.isArray(data)
                ? data.find((m) => m.bad_naam === 'Peuterbad')
                : null;
            ui.zetInputValue('peuterbad-ph', meting?.ph_waarde ?? '');
            ui.zetInputValue('peuterbad-chloor', meting?.chloor_waarde ?? '');
            ui.zetInputValue(
                'peuterbad-filterdruk',
                meting?.filter_druk ?? meting?.filter_druk_in ?? '',
            );
    ...
    _genereerRijCoordinatoren(badNaam, meting) {
        ...
        const vNum = parseFloat(vrij),
            tNum = parseFloat(totaal);
    ...
    async _laadCoordChecklist(datum) {
        ...
        try {
            ...
            console.error('Fout bij laden checklist:', e);
        }
        ...
        form.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
            cb.addEventListener('change', (e) => {
                e.stopPropagation();
                this._scheduleAutoSaveChecklist();
            });
    ...
    async _laadCoordDaggegevens(datum) {
        ...
        try {
            ...
            console.error('Fout bij laden daggegevens:', e);
        }
        ...
        form.querySelectorAll('input').forEach((input) => {
            input.addEventListener('input', (e) => {
                e.stopPropagation();
                this._scheduleAutoSaveDaggegevens();
            });
    ...
    _scheduleAutoSaveDaggegevens() {
        ...
        state.daggegevensAutoSaveTimer = setTimeout(async () => {
            ...
            const payload = {
                datum,
                lucht_temperatuur:
                    parseFloat(document.getElementById('coord-lucht-temp').value) || null,
                bezoekers_vandaag:
                    parseInt(document.getElementById('coord-bezoekers-vandaag').value) || null,
            };
            try {
    ...
    _maakBlokElement(tijdstip, metingen, auteur = '') {
        ...
        const auteurLabel = auteur
            ? `<span style="font-size:13px;font-weight:normal;color:#888;margin-left:10px;">— ${auteur}</span>`
            : '';
        ...
        el.querySelector('.blok-tijdstip-input').addEventListener('change', async (e) => {
            ...
            try {
                await this.app.api.call(
                    `/api/coordinatoren?datum=${datum}&tijdstip=${encodeURIComponent(oudTijdstip)}`,
                    { method: 'DELETE' },
                );
            } catch {
                /* ignore */
        ...
        el.querySelectorAll(
            'input:not(.blok-tijdstip-input):not(.c-chloor-gebonden), select',
        ).forEach((input) => {
            input.addEventListener('input', () =>
                this.app.opslaan.scheduleAutoSaveBlok(el.getAttribute('data-blok-tijdstip')),
            );
    ...
    async _verwijderBlok(tijdstip) {
        if (
            !(await this.app.ui.bevestig({
                tekst: `Meetblok ${String(tijdstip).slice(0, 5)} verwijderen?`,
    ...
    voegNieuwBlokToe() {
        ...
        const g = this.app.state.ingelogdeGebruiker;
        const auteur = g
            ? [g.voornaam, g.achternaam].filter(Boolean).join(' ').trim() || g.inlognaam
            : '';
