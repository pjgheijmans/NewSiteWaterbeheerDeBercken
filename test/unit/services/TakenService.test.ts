...
describe('getTaken — samenstelling', () => {
    beforeEach(() => {
        ...
        actiesRepo.getActies.mockResolvedValue([
            ...
            actie(
                2,
                'Diep',
                'Chloorvoorraad onder 200 liter — Chloor bestellen',
                'chloor_bestellen',
            ),
            actie(
                3,
    ...
    it('laat de filter-rondetaak normaal/overig zonder open alarm', async () => {
        const peuterFilter = (await service.getTaken(DATUM)).find(
            (i) => i.sleutel === 'peuterbad_filter',
        )!;
    ...
    it('houdt een afgevinkt filteralarm in Verplicht (afgehandeld, mét reden)', async () => {
        ...
        actiesRepo.getActies.mockResolvedValue([
            actie(
                1,
                'Diep',
                'Flow Diep onder 250 m³/h — Filter spoelen',
                'filter_spoelen_flow',
                true,
            ),
        ]);
        const diepFilter = (await service.getTaken(DATUM)).find(
            (i) => i.sleutel === 'diep_filter',
        )!;
