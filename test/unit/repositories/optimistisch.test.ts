...
let pool: MockPool;
beforeEach(() => { pool = maakMockPool(); });

const opslaan = (auteur: string | null, verwachteVersie: number | null) =>
    optimistischOpslaan(pool as unknown as Pool, 'metingen_diep_ondiep',
        { bad_id: 1, datum: '2026-05-31' }, { ph_waarde: 7.2, flow: 300 }, auteur, verwachteVersie);

const META = (versie: number) => resultaat([{ versie, auteur: 'Jan', bijgewerkt_op: '2026-05-31T10:00:00' }]);
...
describe('optimistischOpslaan', () => {
    ...
    it('bestaand record met andere versie → 409 conflict (geen INSERT)', async () => {
        ...
        await expect(opslaan('Jan', 2)).rejects.toMatchObject({ status: 409, message: CONFLICT_BERICHT });
