...
new ActieTekstenRepository({} as unknown as Pool).getDefaults().forEach((t) => {
    standaardSjablonen[t.actie_sleutel] = t.sjabloon;
});
...
describe('genereer — Diep/Ondiep drempelwaarden', () => {
    it('maakt een filterdruk-actie aan als het verschil de drempel overschrijdt', async () => {
        ...
        await repo.genereer(1, '2026-05-31', 'Diep', {
            ...
            bad_naam: 'Diep',
            filter_druk_in: 1.0,
            filter_druk_uit: 0.5, // verschil 0.5 > 0.40
    ...
    it('verwijdert de filterdruk-actie als het verschil onder de drempel ligt', async () => {
        ...
        await repo.genereer(1, '2026-05-31', 'Diep', {
            ...
            bad_naam: 'Diep',
            filter_druk_in: 0.6,
            filter_druk_uit: 0.5, // verschil 0.1 < 0.40 → inactief
            flow: 300,
        } as any);
    ...
    it('hanteert een aparte flow-drempel per bad: dezelfde flow triggert Diep wel, Ondiep niet', async () => {
        ...
        await repo.genereer(2, '2026-05-31', 'Ondiep', {
            datum: '2026-05-31',
            bad_naam: 'Ondiep',
            flow: 100,
        } as any);
...
describe('genereer — Peuterbad meetwaarden', () => {
    it('maakt filterdruk- (>1.0) en flow-acties (<4) aan op de peuterbad-drempels', async () => {
        ...
        const drukCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'filter_spoelen_druk',
        );
        const flowCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'filter_spoelen_flow',
        );
...
describe('genereerVerbruik', () => {
    it('maakt een chloor-bestel-actie aan onder de voorraaddrempel', async () => {
        ...
        await repo.genereerVerbruik('2026-05-31', {
            datum: '2026-05-31',
            chemicalien_chloor: 100, // < 200 → actief
        } as any);
        // Eerste stelIn na de baden-query
        const chloorCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'chloor_bestellen',
        );
    ...
    it('maakt zwavelzuur- (<50) en floculant-acties (<10) aan onder hun drempels', async () => {
        ...
        await repo.genereerVerbruik('2026-05-31', {
            datum: '2026-05-31',
            chemicalien_zwavelzuur: 30,
            floculant: 5,
        } as any);
        const zwavelCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'zwavelzuur_bestellen',
        );
        const flocCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'floculant_bijvullen',
        );
...
describe('genereerBezoekers', () => {
    ...
    it('maakt een bezoekers-actie aan voor zowel Diep als Ondiep boven de drempel (750)', async () => {
        ...
        const diepCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'filter_spoelen_bezoekers' && c[1][0] === 1,
        );
        const ondiepCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'filter_spoelen_bezoekers' && c[1][0] === 2,
        );
...
describe('genereerSpoelbeurt', () => {
    ...
    it('telt per bad onafhankelijk: Diep reset op zijn eigen spoelbeurt, Ondiep telt door', async () => {
        ...
        expect(paramsVan(pool.execute, 2)).toEqual([
            '2026-05-31',
            1,
            '2026-05-31',
            'diep_filter',
        ...
        expect(paramsVan(pool.execute, 6)).toEqual([
            '2026-05-31',
            2,
            '2026-05-31',
            'ondiep_filter',
...
describe('genereer — Peuterbad chemicaliën', () => {
    it('maakt een chloor-bijvul-actie aan onder de peuterbad-drempel (10)', async () => {
        ...
        await repo.genereer(3, '2026-05-31', 'Peuterbad', {
            datum: '2026-05-31',
            bad_naam: 'Peuterbad',
            chemicalien_chloor: 8, // < 10 → actief
        } as any);
        const chloorCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'chloor_peuterbad_bijvullen',
        );
        expect(chloorCall).toBeDefined();
    ...
    it('verwijdert de zwavelzuur-bijvul-actie boven de peuterbad-drempel (5)', async () => {
        ...
        await repo.genereer(3, '2026-05-31', 'Peuterbad', {
            datum: '2026-05-31',
            bad_naam: 'Peuterbad',
            chemicalien_zwavelzuur: 9, // >= 5 → inactief
        } as any);
        const zwavelCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][2] === 'zwavelzuur_peuterbad_bijvullen',
        );
...
describe('genereerCoordinatoren', () => {
    it('maakt een gebonden-chloor-actie aan boven de drempel en een peuterbad-aftap-actie als gebruikt', async () => {
        ...
        const gebondenCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'filter_spoelen_gebonden',
        );
        expect(gebondenCall).toBeDefined();
        expect(String(gebondenCall![0])).toMatch(/INSERT INTO acties/i);

        const aftapCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][3] === 'peuterbad_aftappen',
        );
    ...
    it('verwijdert de gebonden-chloor-actie als de waarde onder de drempel ligt', async () => {
        ...
        const gebondenCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][2] === 'filter_spoelen_gebonden',
        );
        expect(gebondenCall).toBeDefined();
    ...
    it('verwijdert de peuterbad-aftap-actie als het bad niet is gebruikt', async () => {
        ...
        await repo.genereerCoordinatoren('2026-05-31');
        const aftapCall = pool.execute.mock.calls.find(
            (c) => Array.isArray(c[1]) && c[1][2] === 'peuterbad_aftappen',
        );
        expect(aftapCall).toBeDefined();
    ...
    it('genereert gebonden-chloor voor elk bad maar aftappen uitsluitend voor Peuterbad', async () => {
        ...
        const gebondenCalls = pool.execute.mock.calls.filter(
            (c) =>
                Array.isArray(c[1]) &&
                (c[1][2] === 'filter_spoelen_gebonden' || c[1][3] === 'filter_spoelen_gebonden'),
        );
        const aftapCalls = pool.execute.mock.calls.filter(
            (c) =>
                Array.isArray(c[1]) &&
