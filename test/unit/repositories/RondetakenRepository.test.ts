...
describe('CATALOGUS', () => {
    it('bevat de vijf belangrijke (kritieke) taken', () => {
        const kritiek = RondetakenRepository.CATALOGUS.filter(
            (t) => t.prioriteit === 'kritiek',
        ).map((t) => t.sleutel);
    ...
    it('koppelt elke taak aan een geldige bad-pagina', () => {
        expect(
            RondetakenRepository.CATALOGUS.every(
                (t) => t.pagina === 'grote-baden' || t.pagina === 'peuterbad',
            ),
        ).toBe(true);
        expect(
            RondetakenRepository.CATALOGUS.find((t) => t.sleutel === 'regelaar_peuterbad')!.pagina,
        ).toBe('peuterbad');
        expect(
            RondetakenRepository.CATALOGUS.find((t) => t.sleutel === 'filters_spraypark')!.pagina,
        ).toBe('peuterbad');
    });
...
describe('getRondetaken', () => {
    ...
    it('markeert taken met een voltooiing als voltooid + metadata', async () => {
        pool.execute.mockResolvedValue(
            resultaat([
                {
                    taak_sleutel: 'regelaar_diep',
                    voltooid_op: '2026-05-31T09:30',
                    voltooid_door: 'Jan',
                },
            ]),
        );
