...
describe('GET /metingen', () => {
    ...
    it('geeft 403 voor coordinator', async () => {
        expect((await request(maakApp('coordinator')).get('/metingen?van=2026-05-01&tot=2026-05-31')).status).toBe(403);
    ...
    it('geeft 500 bij een fout', async () => {
        ...
        expect((await request(maakApp()).get('/metingen?van=2026-05-01&tot=2026-05-31')).status).toBe(500);
    });
});
...
describe('GET /verbruik', () => {
    ...
    it('geeft 403 voor coordinator', async () => {
        expect((await request(maakApp('coordinator')).get('/verbruik?van=2026-05-01&tot=2026-05-31')).status).toBe(403);
    });
});
