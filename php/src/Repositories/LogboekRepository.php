...
/**
 * Port van backend/repositories/LogboekRepository.ts — het waterbeheer-logboek
 * (`logboek`-tabel). Eén regel per (datum, tijdstip); auteur wordt alleen bij de
 * eerste insert vastgelegd, niet overschreven bij een tekst-update.
 */
class LogboekRepository implements ILogboekRepository
{
    ...
    public function save(string $datum, string $tijdstip, string $tekst, ?string $auteur): ?array
    {
        ...
        $stmt = $this->pdo->prepare('SELECT id, auteur FROM logboek WHERE datum = ? AND tijdstip = ?');
    ...
    {
        $stmt = $this->pdo->prepare(
            "SELECT DATE_FORMAT(datum, '%Y-%m-%d') AS datum FROM logboek WHERE id = ?",
        );
        $stmt->execute([$id]);
        $row = $stmt->fetch();
    ...
    {
        $this->pdo->prepare('DELETE FROM logboek WHERE id = ?')->execute([$id]);
    }
}
