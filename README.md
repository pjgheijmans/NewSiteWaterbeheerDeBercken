...
| `metingen_diep_ondiep`| Meetwaarden Diep/Ondiep: pH, chloor, temp, flow, filterdruk    |
...
> `verbruik_diep_ondiep` en `verwarmings_systeem_diep_ondiep` zijn een splitsing van de vroegere `metingen_algemeen` tabel.
...
- Het schema is gesplitst in schone tabellen per domein. `metingen_algemeen` bestaat niet meer; de data zit nu in `verbruik_diep_ondiep` en `verwarmings_systeem_diep_ondiep`.
